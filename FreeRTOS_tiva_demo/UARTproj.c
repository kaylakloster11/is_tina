...
void UART_send_byteA0(uint8_t data){
  ...
  void UART0_IRQHandler(void){
     ...
     // BaseType_t xHigherPriorityTaskWoken = pdFALSE;
      ...
      switch(input){
      case '1':
          xEventGroupSetBits(xUserInEvent, FP_CHECKID_TASK_BIT);
         // portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
          break;
      case '2':
               xEventGroupSetBits(xUserInEvent, FP_ENROLL_TASK_BIT);
               //portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
               break;
      case '3':
               xEventGroupSetBits(xUserInEvent, FP_DELETEID_TASK_BIT);
              // portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
               break;
      case '4':
               xEventGroupSetBits(xUserInEvent, FP_DELETEALL_TASK_BIT);
              // portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
               break;
      }
  }
